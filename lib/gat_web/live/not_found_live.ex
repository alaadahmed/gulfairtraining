defmodule GATWeb.NotFoundLive do
  ...
  import GATWeb.Gettext, only: [gettext: 1]
      ...
      <main class="flex flex-col justify-center flex-grow w-full px-4 mx-auto max-w-7xl sm:px-6 lg:px-8" >
        <div class="flex justify-center shrink-0">
          <%= live_redirect to: Routes.page_path(@socket, :home), class: "inline-flex" do %>
            <span class="sr-only">Gulf Air Training</span>
            <img
              class="h-20"
        ...
        <div class="py-10">
          <div class="text-center">
            <p class="text-sm font-semibold tracking-wide text-indigo-600 uppercase"><%= gettext("404 error") %></p>
            ...
            <div class="mt-6">
              <%= live_redirect to: Routes.page_path(@socket, :home), class: "text-base font-medium text-indigo-600 hover:text-indigo-500" do %>
                <%= gettext("Go back home") %><span aria-hidden="true">&rarr;</span>
              <% end %>
            </div>
          </div>
        </div>
